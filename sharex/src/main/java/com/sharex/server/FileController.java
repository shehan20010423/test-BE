...
import com.sharex.faulttolerance.ServerHealthMonitor;
...
public class FileController {
    ...
    /**
     * HEALTH CHECK: Health endpoint for fault tolerance monitoring
     * GET /health
     *
     * Returns health status of this server.
     * Used by ServerHealthMonitor to check if server is alive.
     */
    @GetMapping("/health")
     ...
     * GET /cluster/health
     *
     * Returns health status of all servers in the cluster.
     * Shows which servers are online, suspected, or offline.
     */
    @GetMapping("/cluster/health")
    public ResponseEntity<?> clusterHealth() {
        Map<String, Object> response = new ConcurrentHashMap<>();
        Map<String, Object> serversHealth = new ConcurrentHashMap<>();

        healthMonitor.getAllServersHealth().forEach((serverId, healthInfo) -> {
            serversHealth.put(serverId, Map.of(
                    "status", healthInfo.getStatus().name(),
                    "online", healthInfo.isOnline(),
                    "lastHeartbeat", healthInfo.getLastHeartbeat().toString(),
                    "missedHeartbeats", healthInfo.getMissedHeartbeats()
            ));
        });

        response.put("servers", serversHealth);
        response.put("onlineCount", healthMonitor.getOnlineServerCount());
        response.put("totalServers", clusterConfig.getAllServers().size());
        response.put("monitorRunning", healthMonitor.isRunning());
